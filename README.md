...
另有 **HTML 网页版**（secure_link_v3.html）：浏览器点对点加密通讯 + 内置游戏厅，无需安装 Python。
...
## HTML 网页版（secure_link_v3.html，推荐）

双击用 Chrome/Edge 打开即可，无需安装任何东西。单文件（内置两款游戏合集），可完全离线使用。

**🎮 内置游戏厅（老式小游戏）**：
- 一键打开游戏厅：**复古迷你游戏合集 · 45 款**（贪吃蛇 / 俄罗斯方块 / 打砖块 / 扫雷 / 2048 / 飞机大战 / 打地鼠 / 赛车 / 泡泡龙 / 消消乐…）
- **100合1 复古游戏**（掌机式百合一，方向键 + A/B 键）
- 游戏完全内嵌于单文件，不联网也能玩；边等文件传输边打游戏

**🆔 永久 ID 系统（快速直连）**：
- 每个浏览器首次打开自动生成永久 ID（格式 SL-XXXX，存 localStorage，永不改变）
- 双方都连接信令服务器后，**只需知道对方 ID 即可 1v1 直连**，无需复制粘贴信令
- 离线场景：server.py 跑在局域网内任一机器，双方连它即可用 ID 互找

**📡 三种连接模式**：
1. **手动信令（完全离线）**：局域网内点对点直连，信令用微信/QQ 复制或扫码交换，断外网也能用
2. **服务器房间**：群聊（mesh 最多 8 人）/ 随机匹配 / 私密房间 / 房间列表
3. **ID 直连**：连接服务器后凭对方永久 ID 快速 1v1

**优化与修复**：
- 修复“点击接收方无反应”：点“我是接收方”立即弹出信令交换界面并聚焦输入框，粘贴 offer 自动生成应答
- 防卡顿：聊天消息上限 300 条自动裁剪、进度条 requestAnimationFrame 节流、文件 16MB 背压控制
- 纯局域网模式开关（不请求外网 STUN，完全离线更快）
...
**加密**：口令 + **PBKDF2(60万次迭代, SHA-256)** 派生 AES-256-GCM 会话密钥，盐随信令交换，每次会话不同；文本、文件头、文件数据块全加密，口令错误或数据被篡改时提示解密失败。强度高于 Python 版（scrypt n=2^15）。
